...
const LICENSEE = {
  name: "ИП Курочкин Артем Андреевич",
...
function DocumentPreview({ data }: { data: Contract }) {
  ...
  return <article className="paper" id="document-preview">
    ...
    <h3>7. ПОДПИСАНИЕ ДОГОВОРА</h3><p><b>7.1.</b> Договор заключается в письменной форме и подписывается Сторонами собственноручно. Каждый экземпляр, подписанный Стороной, имеет одинаковую юридическую силу.</p><p><b>7.2.</b> Датой договора считается дата его создания, указанная в заголовке документа. Изменения и дополнения оформляются в письменной форме и подписываются обеими Сторонами.</p><p><b>7.3.</b> Подписанный экземпляр передаётся другой Стороне лично, курьером или иным согласованным способом.</p>
    ...
    <h3>9. РЕКВИЗИТЫ И ПОДПИСИ</h3><div className="signature-grid"><div><span>ЛИЦЕНЗИАР</span><strong>{display(data.licensorName)}</strong><p>Паспорт: {display(data.passportSeriesNumber)}<br />Выдан: {display(data.passportIssuedBy)}<br />Дата выдачи: {formatDate(data.passportIssueDate)}<br />Банковские реквизиты: {display(data.bankDetails)}<br />Email: {display(data.email)}</p><i>________________ / подпись</i></div><div><span>ЛИЦЕНЗИАТ</span><strong>{LICENSEE.name}</strong><p>Дата рождения: {LICENSEE.birthDate}<br />Паспорт: {LICENSEE.passport}<br />Выдан: {LICENSEE.passportIssuedBy}<br />ИНН: {LICENSEE.inn}<br />Расчётный счёт: {LICENSEE.account}<br />Банк: {LICENSEE.bank}<br />БИК: {LICENSEE.bik}<br />Город банка: {LICENSEE.bankCity}<br />Корр. счёт: {LICENSEE.correspondentAccount}<br />Email: {LICENSEE.email}</p><img className="signature-image" src={SIGNATURE_SRC} alt="Подпись Курочкина Артема Андреевича" /><i>________________ / подпись</i></div></div>
