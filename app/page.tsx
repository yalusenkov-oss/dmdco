...
function DocumentPreview({ data }: { data: Contract }) {
  ...
  return <article className="paper" id="document-preview">
    <div className="paper-topline"><span>PFVMUSIC</span><span>Лицензионный договор PFVMUSIC</span></div>
...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-mark">PFV</div><div><strong>PFVFORM</strong><span>Конструктор договоров</span></div></div><div className="top-actions"><span className="saved-state"><span className="status-dot" />{savedAt ? `Сохранено в ${savedAt}` : "Автосохранение"}</span><button className="ghost-button" onClick={exportDraft}>Экспорт черновика</button><button className="primary-button" onClick={() => window.print()}>Печать / PDF</button></div></header>
    <div className="workspace"><aside className="sidebar"><div className="sidebar-intro"><span className="eyebrow">ЛИЦЕНЗИОННЫЙ ДОГОВОР PFVMUSIC</span><h1>Заполните только<br /><em>нужное.</em></h1><p>Форма повторяет поля актуального договора PFVFORM: ничего лишнего для дистрибуции.</p></div><div className="progress-card"><div className="progress-head"><span>Заполнено</span><b>{completeness}%</b></div><div className="progress-track"><span style={{ width: `${completeness}%` }} /></div><small>Черновик сохраняется на этом устройстве</small></div><nav className="steps-nav" aria-label="Разделы договора">{sections.map((section) => <button key={section.id} className={active === section.id ? "active" : ""} onClick={() => jumpTo(section.id)}><span>{section.number}</span>{section.label}<i>→</i></button>)}</nav><div className="tip-card"><span>✦</span><div><b>Состав договора</b><p>Основной договор, Приложение № 1 и Дополнительное соглашение.</p></div></div><div className="sidebar-foot">Источник: PFVFORM<br /><span>license-v2.2 · 30.07.2026</span></div></aside>
